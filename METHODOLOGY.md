...
**Note:** The workbook also contains **Current** (original extraction) and **Future** (future scenario assumptions) sheets. The automated script uses the **Current_v2** sheet, which refines the original pier proportions to reflect that SICT piers (51, 52, 53) do not handle Liquid-Bulk cargo.
...
If "Milled grain prods." has:
- Total tonnage: 10,000 tons
- `Primary_Cargo_Type`: Containers
- `Containers_Proportion`: 0.7
- `Alternative_Cargo_Type`: Dry-Bulk

The distribution would be:
- 7,000 tons (70%) distributed to piers based on their Container handling proportions
- 3,000 tons (30%) distributed to piers based on their Dry-Bulk handling proportions
...
### 6.4 Console Output Summary

The script prints a summary to the console for quick reference:
...
Top 5 Commodities by Tonnage (Scaled Model):
  ...
  2. [Commodity Name]: XXX,XXX tons (XX.X%)
  3. [Commodity Name]: XXX,XXX tons (XX.X%)
  4. [Commodity Name]: XXX,XXX tons (XX.X%)
  5. [Commodity Name]: XXX,XXX tons (XX.X%)
```

This summary provides key statistics for presentations and executive summaries.

### 6.5 Running the Analysis

To regenerate the analysis results after updating the FAF data:

```bash
python Script/analyze_sict_results.py
```

The script will:
1. Load the three deliverable sheets from `FAF_Hawaii_Region_2024.xlsx`
2. Perform all analyses
3. Save results to `SICT_Analysis_Results.xlsx`
